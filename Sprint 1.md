...
- [Backend] Set up PostgreSQL with Docker — database container running locally; docker-compose configuration for PostgreSQL.
- [Backend] Implement POST /auth/register — user registration with email and password;Implement POST /auth/login — user login with email and password
- [Backend] Create User model — defines user data structure with email and password; includes relationships to hotels and flights.
- [Backend] Create Hotel model — defines hotel data structure with fields: hotel_name, city, country, check_in, check_out, price, notes, latitude, longitude; Implement Hotel CRUD APIs — full create, read, update, delete operations for hotels; protected by JWT authentication; user can only access their own hotels.

## Not completed (and why)

- Logout Design：
  Reason：JWT is stateless, the token is stored on the client side. Logout can be handled simply by deleting the token from local storage on the frontend. The token will automatically expire after 7 days, so no server-side logout endpoint is required.
